...
$(document).ready(function () {
    ...
    if (targeta_filtros.$cabecera != "") {
        ...
        if (targeta_filtros.$estado.val() == "CERRADO" | targeta_filtros.$estado.val() == "EN TRANSITO"){
...
TargetaFormulario.prototype.init = function () {
    ...
    this.$almacen_origen.on('change', this, this.validar_AlmacenOrigen)
    this.$almacen_destino.on('change', this, this.validar_AlmacenDestino)
    
}
TargetaFormulario.prototype.validar_Almacenes = function () {
    almacen_destino = this.$almacen_destino.val()
    almacen_origen = this.$almacen_origen.val()
    if (almacen_origen == almacen_destino){
       targeta_filtros.$boton_guardar.attr("disabled", true)
       return true
    }
    else {
        return false
...
TargetaFormulario.prototype.validar_AlmacenOrigen = function (e) {
    ...
    $.ajax({
            ...
            success: function (response) {
                if(response.results[0].estado!="DESHABILITADO"){
                    ...
                    alertify.error("Almacén Inhabilitado. Por favor seleccione otro.")
                    
                }
                res = targeta_filtros.validar_Almacenes()
                if (res){
                    e.data.$boton_guardar.attr("disabled", true)
                    alertify.error("Almacén Destino no puede ser el mismo que Almacen Origen")
                }
            },
            error: function () {
...
TargetaFormulario.prototype.validar_AlmacenDestino = function (e) {
    
    $.ajax({
            url: url_almacenes,
            method: "GET",
            data: {
            "id": targeta_filtros.$almacen_destino.val()
            },
            success: function (response) {
                if(response.results[0].estado!="DESHABILITADO"){
                    e.data.$boton_guardar.attr("disabled", false)
    ...
    e.data.Inicializar_CeldasExcel();
        ...
        targeta_resultados.grid.kfuente_datos_excel.fetch(function () {
            ...
            kendo.saveAs({
                ...
                fileName: "Traspasos_Detalle.xlsx",
            });
        });
}
