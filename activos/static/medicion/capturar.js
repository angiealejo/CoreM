...
function Tabla () {
    ...
    this.$boton_ver_observacion = $("a[data-observacion='ver']")
...
Tabla.prototype.validarCampos = function (_event_owner) {
    ...
    if (_event_owner.parents("tr").find('.fecha-lectura').val() == '') {
        console.log(_event_owner.parents("tr").find('.fecha-lectura').val())
    ...
    if (_event_owner.parents("tr").find('.lectura').val() == '') {
...
Tabla.prototype.click_tituloOdometro = function (e) {
    ...
    odometro_texto = e.data.event_owner.attr('data-descripcion')
    tabla.modal_historico.$odometro_texto.text("Odómetro: " + odometro_texto)
    tabla.modal_historico.odometro = id_odometro
    grid_resultados.event_owner_id = e.data.event_owner.attr('data-id-medicion')
    grid_resultados.clasificacion = e.data.event_owner.attr('data-clasificacion')
...
ModalObservacion.prototype.init = function () {
    ...
    this.$observaciones.on('keypress', this, this.validarTecla)
    this.$boton_guardar.on('click', this, this.nuevo)
}
ModalObservacion.prototype.validarTecla = function (e) {
    if (e.which == 13) {
        e.data.nuevo(e)
    }
}
...
// GridHistorico.prototype.numberEditor = function (container, options) {
    ...
    return {
        ...
        save: function(e) {
            ...
            if (e.model.equipo){
                ...
                grid_resultados.kfuente_datos.read()
                grid_resultados.kfuente_datos_excel.read()
                grid_resultados.actualizar_Tabla(grid_resultados.clasificacion, e.model.pk, grid_resultados.event_owner_id, lectura, observaciones)
...
GridHistorico.prototype.actualizar_Tabla = function (clasificacion, id_medicionActualizada, id_Disparador, lectura, observaciones) {

    if (id_medicionActualizada == id_Disparador) {

        if (clasificacion == "NUM"){

            tabla.event_owner.parents('tr').find('.ultima-lectura').text(lectura)
        }
        else {
            tabla.event_owner.parents('tr').find('.ultima-lectura').text(observaciones)
        }
    }
